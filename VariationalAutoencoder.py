...
class VA:
    ...
    def sample_from_p_z(self, num_samples):
        return np.random.multivariate_normal(size=num_samples, mean=self.prior_mu,
                                             cov=T.exp(2 * self.log_prior_sigma).eval() * np.eye(self.dimZ))
    ...
    def createGradientFunctions(self):
        ...
        if self.continuous:
            ...
            logpxz = T.sum(-(0.5 * np.log(2 * np.pi) + log_sigma_decoder) - 0.5 * (
    ...
    def updateParams(self, totalGradients, N, current_batch_size):
        ...
        for i in range(len(self.params)):
            ...
            self.params[i] += self.learning_rate / np.sqrt(self.h[i]) * (
                        totalGradients[i] - prior * (current_batch_size / N))
